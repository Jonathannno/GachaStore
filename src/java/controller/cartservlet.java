...
public class cartservlet extends HttpServlet {
    ...
    // 1. Get the quantity from the form (if missing, default to 1)
    String qtyInput = request.getParameter("quantity");
    int requestedQty = (qtyInput != null) ? Integer.parseInt(qtyInput) : 1;

    int currentStock = getProductStock(conn, productId);
    int currentInCart = getQtyInCart(conn, userId, productId);

    // 2. Check stock using the REAL requested quantity
    if (currentInCart + requestedQty > currentStock) {
        response.sendRedirect("Cart.jsp?error=exceed_stock&max=" + currentStock);
        return;
    }

    // 3. Update SQL to use the 'requestedQty' variable
    String sql = "INSERT INTO cart (user_id, product_id, quantity) VALUES (?, ?, ?) "
               + "ON DUPLICATE KEY UPDATE quantity = quantity + ?";
    PreparedStatement ps = conn.prepareStatement(sql);
    ps.setInt(1, userId);
    ps.setInt(2, productId);
    ps.setInt(3, requestedQty); // For new items
    ps.setInt(4, requestedQty); // For existing items (adding to what's already there)
    ps.executeUpdate();
}
